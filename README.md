...
**Введение**
**T-Match** — это сервис знакомств и поиска друзей, который подбирает людей на основе их реального финансового поведения (истории транзакций). Вместо заполнения долгих анкет проект использует машинное обучение (поиск lookalike-пользователей по категориям трат и любимым заведениям), чтобы находить людей с похожим стилем жизни. Кроме того, встроенная LLM генерирует для пользователей понятные и безопасные объяснения причин их совместимости. Цель проекта — упростить поиск единомышленников, основываясь на честных данных о привычках и интересах.
